...
struct SphereObject
{
	...
	SphereObject(glm::vec3 inputSphereCenter, glm::vec3 inputColor, float inputRadiusFloat, Material mat, bool isLightSource)
	{
		s = Sphere(inputSphereCenter, inputRadiusFloat);
		color = inputColor;
		m = mat;
...
float Ray_VS_Sphere_WithIntersectionNormal(Ray& r, Sphere s, glm::vec3 & normal)
{
	//t values to compare which one is the closest
	float current_t = INFINITY;
	//return black by default
